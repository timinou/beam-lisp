defmodule BeamLisp.PreludeTest do
  ...
  test "map, filter, reduce are written in beam-lisp" do
    # These fns are lazy, so the assertion is about the VALUES they
    # produce — realize/1 forces the seq before comparing. Comparing a
    # lazy value with Elixir's == was always an accident of how these
    # were written, never a claim that the result was a list.
    assert realize(eval("(map inc [1 2 3])")) == [2, 3, 4]
    assert realize(eval("(map (fn [x] (* x x)) [1 2 3])")) == [1, 4, 9]
    assert realize(eval("(filter (fn [x] (> x 2)) [1 2 3 4])")) == [3, 4]
  ...
  test "prelude fns compose with interop" do
    assert realize(eval("(map String/upcase [\"a\" \"b\"])")) == ["A", "B"]
  end
