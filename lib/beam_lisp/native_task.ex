defmodule BeamLisp.NativeTask do
  @moduledoc """
      ...
      case System.find_executable("cargo") do
        nil ->
          ...
          if File.exists?(Path.join(memo_dir, "Cargo.toml")) and
            ...
            raise(
              "LazySeq requires the lazy_memo native runtime. Install cargo and run bl build; no current native artifact is available."
            )
          end

          IO.puts("no cargo on PATH — native backends will report unavailable")
  ...
  defp compile(crate, crate_dir) do
    ...
    case System.cmd("cargo", ["build", "--release"],
      ...
      {out, code} ->
        ...
        raise("cargo build failed for #{crate} (exit #{code}):\n#{out}")
    end
  end
